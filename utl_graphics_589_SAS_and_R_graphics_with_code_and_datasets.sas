...
*                        _               _     _
 _ __ ___  ___ ___ _ __ | |_    __ _  __| | __| |___
| '__/ _ \/ __/ _ \ '_ \| __|  / _` |/ _` |/ _` / __|
| | |  __/ (_|  __/ | | | |_  | (_| | (_| | (_| \__ \
|_|  \___|\___\___|_| |_|\__|  \__,_|\__,_|\__,_|___/
...
%pdfbeg(pdf=d:/pdf/&pgm._002a.pdf);
%codebegin;
cards4;

* uses WPS proc R substitute IML/R or pure R;
options validvarname=upcase;
libname sd1 "d:/sd1";
data sd1.have(drop=i);
call streaminit(123);
do i=1 to 100;
  lat =int(5*(rand("Uniform")))+1;
  lon =int(5*(rand("Uniform")))+1;
  temp =int(10*(rand("Uniform")))+1;
  pop =int(40*(rand("Uniform")))+1;
  output;
end;
run;quit;
...
head(have);
pdf("d:/pdf/beeswarm.pdf");
ggplot(aes(x=LON, y=LAT, col=TEMP, size=POP),  data = have)+
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=13pt);
%pdfbeg(pdf=d:/pdf/&pgm._003a.pdf);
%codebegin;
cards4;
data Heart;
...
xArrowO=-1.85; yArrowO=-3.6; xArrow=2.5; yArrow=.75;
output; xArrowO=.; yArrowO=.; xArrow=.; yArrow=.;
xArrowO2=1.25; yArrowO2=-.5; xArrow2=2.5; yArrow2=.75;
output; xArrowO2=.; yArrowO2=.; xArrow2=.; yArrow2=.;
xText=0; yText=-1.25; Text='MOM'; output; * Text;

proc sgplot data=Heart aspect=1 nowall noautolegend noborder;

vector x=xArrow y=yArrow /
       xorigin=xArrowO yorigin=yArrowO
lineattrs=(color=darkgreen thickness=10pt) arrowheadshape=barbed;
scatter x=xMarker y=yMarker /
markerattrs=(symbol=circlefilled size=20pt color=darkgreen);
...
vector x=xArrow2 y=yArrow2 /
       xorigin=xArrowO2 yorigin=yArrowO2
lineattrs=(color=darkgreen thickness=10pt) arrowheadshape=barbed;
text x=xText y=yText text=Text / backlight=1 strip
textattrs=(color=black size=72pt weight=bold);
...
%utl_pdflan100;
%pdfbeg(pdf=d:/pdf/&pgm._003b.pdf);
proc sgplot data=Heart aspect=1 nowall noautolegend noborder;
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=13pt);
%pdfbeg(pdf=d:/pdf/&pgm._004a.pdf);
%codebegin;
cards4;
...
ods pdf file="d:/pdf/&pgm._004b.pdf";
...
scatter x=x1 y=y1 / colormodel=(lightgreen green darkgreen)
colorresponse=rnum markerattrs=(symbol=circlefilled);
polygon x=x y=y id=polygon / fill fillattrs=(color=white)
outline lineattrs=(color=black thickness=3pt);
polygon x=x y=y id=polygon / fill
fillattrs=(color=white) nooutline;
inset "MEMORIAL DAY 2017" / position=bottom
textattrs=(color=white weight=bold size=28pt);
run;quit;
ods pdf close;
ods graphics off;
;;;;
...
%pdfbeg(pdf=d:/pdf/&pgm._004b.pdf);
ods graphics on / height=5in width=5in;  /* Generate image */
proc sgplot aspect=1 noautolegend noborder pad=0 nowall ;
xaxis display=none offsetmin=0 offsetmax=0;
yaxis display=none offsetmin=0 offsetmax=0;
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._005a.pdf);
%codebegin;
cards4;
options validvarname=upcase;
libname sd1 "d:/sd1";
...
have;
pdf("d:/pdf/&pgm._005b.pdf",7,5);
ScatterHistC(have, "SEPALLENGTH", "PETALLENGTH", "SPECIES",
...
have;
pdf("d:/pdf/&pgm._005b.pdf",7,5);
ScatterHistC(have, "SEPALLENGTH", "PETALLENGTH", "SPECIES", title = "Petal length vs Sepal length");
endsubmit;
run;quit;
');
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._006a.pdf);
%codebegin;
cards4;
%utl_submit_r64('
source("c:/Program Files/R/R-3.3.1/etc/Rprofile.site",echo=T);
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._007a.pdf);
%codebegin;
cards4;
%utl_submit_py64('
...
plt.xlabel("time");
plt.ylabel("my overall health");
fig.savefig("d:/pdf/ymr_pdf_007b.pdf");
');
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._008a.pdf);
%codebegin;
cards4;
data stars(keep=x y);   * Generate x/y points for 50 stars ;
do r=1 to 9;            * 9 rows of stars ;
  y=1-r*0.0538;
  do x=0.0633*(1+mod(r-1,2)) to 11*0.0633 by 2*0.0633;
   * Odd rows have 6 stars, even have 5 (offset by 1) ;
    output;
  end;
end;
run;quit;

data stripes(keep=spolygon sx sy);       * Generate x/y points for 7 ;
...
data canton(keep=cpolygon cx cy); * Generate x/y points (blue rectangle) ;
...
data flag;                  * Combine 3 datasets used to plot U.S. flag  1 ;
set stars stripes canton;
run;quit;
ods listing  gpath="d:/pdf";
ods graphics on / reset=index border=off imagefmt=gif
    antialias height=5in width=9.5in reset=index imagename="ym_pdf_008b";
...
* Stripes ("Old Glory Red");
polygon x=sx y=sy id=spolygon / fill fillattrs=(color=CXB22234) nooutline;
* Canton ("Old Glory Blue");
polygon x=cx y=cy id=cpolygon / fill fillattrs=(color=CX3C3B6E) nooutline;
* Stars (White);
scatter x=x y=y / markerattrs=(symbol=unistar color=CXFFFFFF size=38pt);
run;quit;
ods graphics off;
;;;;
run;quit;
...
ods listing  gpath="d:/pdf";
ods graphics on / reset=all reset=index border=off imagefmt=png  /* Plot American flag */
    antialias height=5in width=9.5in imagename="ymr_pdf_008b";
proc sgplot data=flag noautolegend noborder pad=0;
symbolchar name=uniStar char='2605'x;    /* Unicode value for 5-pointed star */
xaxis display=none offsetmin=0 offsetmax=0 values=(0 1.9);
yaxis display=none offsetmin=0 offsetmax=0 values=(.005 1);
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._009a.pdf);
%codebegin;
cards4;
options validvarname=upcase;
libname sd1 "d:/sd1";
...
ods pdf file="d:/pdf/&pgm._009a.pdf";
...
%utl_pdflan100(fixedfont=11pt);
ods pdf file="d:/pdf/&pgm._009b1.pdf";
*SAS;
options orientation=landscape;
ods graphics on / height=9in width=7in;
ods pdf file="d:/pdf/&pgm._009b1.pdf";
...
options ls=171 ps=66;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._010a.pdf);
%codebegin;
cards4;
options validvarname=upcase;
libname sd1 "d:/sd1";
...
options ls=171 ps=88;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._012a.pdf);
%codebegin;
cards4;
options validvarname=upcase;
libname sd1 "d:/sd1";
data sd1.iris;
   retain sepallength sepalwidth species;
  set sashelp.iris
   (keep=species sepallength sepalwidth);
run;quit;
%utl_submit_wps64('
options set=R_HOME "C:/Program Files/R/R-3.3.1";
libname sd1 sas7bdat "d:/sd1";
* Largest convex hulls that can contain 95% of the points;
proc r;
submit;
source("c:/Program Files/R/R-3.3.1/etc/Rprofile.site",echo=T);
library(plyr);
...
find_bag = function(x,alpha=alph) {
  ...
  x2old[chull(x2old),] };
  bags <- ddply(iris, "SPECIES", find_bag, alpha=alph);
  plot <- ggplot(data = iris, aes(x = SEPALLENGTH,
  y = SEPALWIDTH, colour=SPECIES, fill = SPECIES)) +
  geom_point() +
  geom_polygon(data = bags, alpha = 0.5) +
  labs(x = "SEPALLENGTH", y = "SEPALWIDTH");
pdf("&pgm._012b.pdf",10,7);
plot;
endsubmit;
run;quit;
');
...
options ls=171 ps=88;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._014a.pdf);
%codebegin;
cards4;
...
proc sql;
    ...
    create table plotb as
        select type,percent,percent/sum(percent) as ppt
    ,sum(percent) as othpct from frq where percent lt 8;
quit;
%let dist=2;
data plota2 (drop=text xtext ytext) texta (keep=text xtext ytext);
    ...
    if type=' ' then do;
        ...
        series=0;
        do i = (2*constant('pi'))*(-percent/2) to
           (2*constant('pi'))*(percent/2) by (2*constant('pi'))*(percent/50);
            x=1*cos(i);
            y=1*sin(i);
            output plota2;
        end;
        series=1;x=x*(&dist/cos(percent*constant('pi')));
        y=y*(&dist/cos(percent*constant('pi')));output plota2;
        endpoint=percent/2;
    end;
    else do;
        id=id+1;
        text=strip(put(percent*100,12.0))||'%';
        xtext=0.6*cos(2*constant('pi')*(percent/2+endpoint));
       ytext=0.6*sin(2*constant('pi')*(percent/2+endpoint));output texta;
        series=1;x=0;y=0;output plota2;
        series=0;
        do i = (2*constant('pi'))*(endpoint) to
            (2*constant('pi'))*(percent+endpoint) by (2*constant('pi'))*(percent/50);
            x=1*cos(i);
            y=1*sin(i);
            output plota2;
        end;
...
data plotb2(drop=text xtext ytext) textb (keep=text xtext ytext);
    ...
    xtext=&dist+0.5*(&dist*tan(othpct/100*constant('pi')))
         *cos(2*constant('pi')*(ppt/2+endpoint));
    ytext=0.5*(&dist*tan(othpct/100*constant('pi')))*
         sin(2*constant('pi')*(ppt/2+endpoint));output textb;
    series=1;x=&dist;y=0;output plotb2;
    series=0;
    do i = (2*constant('pi'))*(endpoint) to (2*constant('pi'))*
        (ppt+endpoint) by (2*constant('pi'))*(ppt/50);
...
proc template;
    ...
    layout overlay / xaxisopts=(display=none
       linearopts=(viewmin=-1.1 viewmax=4))
        yaxisopts=(display=none linearopts=(viewmin=-1.1 viewmax=1.1));
        /*Left Pie Chart*/
        polygonplot x=eval(ifn(circle^=2,x,.)) y=y id=id /
        outlineattrs=(color=black pattern=1) display=(fill) group=type name='p1';
        seriesplot x=eval(ifn(series=1 and circle^=2,x,.)) y=y /
        lineattrs=(color=black pattern=1) group=id;
        polygonplot x=eval(ifn(circle=2,x,.)) y=y id=id /
        outlineattrs=(color=black pattern=1) display=(fill) group=type name='p2';
        seriesplot x=eval(ifn(series=1 and circle=2,x,.)) y=y /
        lineattrs=(color=black pattern=1) group=id;
        DRAWOVAL X=0 Y=0 WIDTH=2 HEIGHT=2 / display=(outline)
        outlineattrs=(color=black) heightunit=data
            drawspace=datavalue widthunit=data;
        DRAWOVAL X=&dist Y=0 WIDTH=%sysevalf(&radius) HEIGHT=%sysevalf(&radius) /
        display=(outline) outlineattrs=(color=black) heightunit=data
            drawspace=datavalue widthunit=data;

        textplot x=xtext y=ytext text=text / textattrs=(size=16pt) position=center;
        discretelegend 'p1'  / exclude=(' ') location=inside halign=right
        valign=center border=false across=1 valueattrs=(size=18pt)
            displayclipped=true;
    endlayout;
...
ods listing  gpath="d:/pdf";
ods graphics on / reset=all reset=index border=off imagefmt=png  /* Plot American flag */
    antialias height=5in width=9.5in imagename="ymr_pdf_014b";
...
options ls=171 ps=88;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._014b.pdf);
proc freq data=sashelp.cars noprint;
    table type / out=frq;
    where type ^='Hybrid';
run;
...
options ls=171 ps=88;
%utl_pdflan100(fixedfont=11pt);
%pdfbeg(pdf=d:/pdf/&pgm._015a.pdf);
ods escapechar='$';
%codebegin;
cards4;
data have;
...
cards;
^{unicode 2764} ^{unicode 263A} ^{unicode 2605} ^{unicode 2602}
...
ods pdf file="d:/pdf/ymr_pdf_015b.pdf" ;
ods escapechar='^';
...
%macro utl_pdfcombine(files, file12=merge1.pdf);

    filename cmd pipe "d:\pdf\tmp\gswin64c.exe -dNOPAUSE -sDEVICE=pdfwrite -sOUTPUTFILE=&file12 -dBATCH &files";
    data _null_;
      infile cmd;
      input;
      putlog _infile_;
...
x "cd d:\pdf\tmp";
%utl_pdfcombine(files=ymr_pdf_002a.pdf ymr_pdf_002b.pdf ymr_pdf_003a.pdf ymr_pdf_003b.pdf,file12=merge1.pdf);
%utl_pdfcombine(files=merge1.pdf ymr_pdf_004a.pdf ymr_pdf_004b.pdf ymr_pdf_005a.pdf ymr_pdf_005b.pdf,file12=merge2.pdf);
%utl_pdfcombine(files=merge2.pdf ymr_pdf_006a.pdf ymr_pdf_006b.pdf ymr_pdf_007a.pdf ymr_pdf_007b.pdf ymr_pdf_008a.pdf ymr_pdf_008b.pdf,file12=merge3.pdf);
%utl_pdfcombine(files=merge3.pdf ymr_pdf_009a.pdf ymr_pdf_009b1.pdf ymr_pdf_009b2.pdf ymr_pdf_010a.pdf ymr_pdf_012a.pdf,file12=merge4.pdf);
%utl_pdfcombine(files=ymr_pdf_012b.pdf ymr_pdf_014a.pdf ymr_pdf_014b.pdf ymr_pdf_015a.pdf ymr_pdf_015b.pdf merge4.pdf,file12=merge5.pdf);
%utl_pdfcombine(files=utl_graphics_400_SAS_and_R_graphics_with_code_and_datasets.pdf merge5.pdf,
file12=utl_graphics_598_SAS_and_R_graphics_with_code_and_datasets.pdf );


* drop the last page -psge 592;
x "cd d:\pdf\tmp";
x "gswin64c -sDEVICE=pdfwrite -dNOPAUSE -dQUIET -dBATCH -dFirstPage=1 -dLastPage=591 -sOutputFile=output1.pdf utl_graphics_598_SAS_and_R_graphics_with_code_and_datasets.pdf";
x "gswin64c -sDEVICE=pdfwrite -dNOPAUSE -dQUIET -dBATCH -dFirstPage=1 -dLastPage=585 -sOutputFile=outputa.pdf output1.pdf";
x "gswin64c -sDEVICE=pdfwrite -dNOPAUSE -dQUIET -dBATCH -dFirstPage=587 -dLastPage=587 -sOutputFile=outputb.pdf output1.pdf";
x "gswin64c -sDEVICE=pdfwrite -dNOPAUSE -dQUIET -dBATCH -dFirstPage=589 -dLastPage=591 -sOutputFile=outputc.pdf output1.pdf";

* put the pieces back together;
%utl_pdfcombine(files=outputa.pdf outputb.pdf outputc.pdf,file12=merge7.pdf);
...
x "gswin64c -dNOPAUSE -sDEVICE=pdfwrite -sOUTPUTFILE=merge.pdf -dBATCH ymr_pdf_002a.pdf ymr_pdf_002b.pdf";
